...
$heroImage = $hero['image_url'] ?? 'https://images.unsplash.com/photo-1541417904950-b855846fe074?auto=format&fit=crop&w=1600&q=80';
$heroAlt = $hero['image_alt'] ?? 'Image de couverture du dossier principal';
...
<section class="relative h-[70vh] min-h-[500px] w-full overflow-hidden bg-primary-container">
    ...
    <div class="absolute inset-0 flex flex-col justify-end p-6 md:p-16">
        <div class="max-w-4xl space-y-5">
            <span class="inline-flex items-center gap-2 bg-on-tertiary-container px-3 py-1 text-white font-label text-[10px] font-bold tracking-[0.3em] uppercase">Priority Alpha Analysis</span>
            ...
            <?php if (!empty($hero['slug'])): ?>
                <a href="/article/<?= e((string) $hero['slug']) ?>" class="inline-block bg-white text-primary px-8 py-3 text-[11px] font-label font-black tracking-widest uppercase hover:bg-surface-container-high">Full report</a>
            <?php endif; ?>
        </div>
    </div>
</section>
...
<section class="px-6 md:px-12 py-16 max-w-7xl mx-auto">
    <div class="flex items-baseline justify-between mb-10 border-b border-outline-variant pb-4">
        <div>
            ...
            <p class="font-label text-[10px] tracking-[0.2em] uppercase opacity-60 mt-2">Brief: sélection rapide des publications clés sur la situation en Iran.</p>
        </div>
        ...
        <?php foreach ($latestCards as $item): ?>
            ...
            <article class="flex flex-col group">
                <div class="mb-5 relative h-48 bg-surface-container overflow-hidden">
                    <img
                        src="<?= e((string) ($item['image_url'] ?? 'https://images.unsplash.com/photo-1518544866330-95a9f2f8fcd0?auto=format&fit=crop&w=900&q=75')) ?>"
                        alt="<?= e((string) ($item['image_alt'] ?? ('Illustration de ' . ($item['title'] ?? 'article')))) ?>"
                ...
                <a href="/article/<?= e((string) $item['slug']) ?>" class="mt-auto flex items-center gap-2 font-label text-[10px] font-bold tracking-[0.2em] uppercase text-primary border-b border-primary w-fit pb-1">Open dispatch</a>
            </article>
        <?php endforeach; ?>
    </div>
</section>
...
<section class="px-6 md:px-12 pb-16 max-w-7xl mx-auto">
    <div class="border border-outline-variant bg-surface-container-lowest p-6 md:p-8 grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div>
            <h2 class="font-headline text-3xl font-bold tracking-tight mb-3">PHOTO BRIEFING</h2>
            <p class="font-body text-sm text-on-surface-variant leading-relaxed mb-6">Détail de l’image principale affichée en tête de page, pour contextualiser la source et le type visuel utilisé.</p>
            <dl class="space-y-3">
                <div>
