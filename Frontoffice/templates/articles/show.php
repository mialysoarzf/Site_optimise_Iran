        ...
        <?php if (!empty($images)): ?>
            <section class="mb-10 space-y-5" aria-label="Images de l'article">
                <?php foreach ($images as $image): ?>
                    <img src="<?= e((string) $image['url']) ?>" alt="<?= e((string) $image['alt']) ?>" class="w-full h-auto object-cover border border-outline-variant" loading="lazy">
                <?php endforeach; ?>
            </section>
        <?php endif; ?>
            ...
            <?php foreach ($related as $item): ?>
                ...
                <article class="border border-outline-variant p-5 bg-surface-container-lowest">
                    <?php if (!empty($item['image_url'])): ?>
                        <img src="<?= e((string) $item['image_url']) ?>" alt="<?= e((string) ($item['image_alt'] ?? ('Illustration de ' . $item['title']))) ?>" class="w-full h-40 object-cover mb-4" loading="lazy">
                    <?php endif; ?>
