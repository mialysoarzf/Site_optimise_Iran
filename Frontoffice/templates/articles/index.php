<section class="page-section">
    ...
    <p class="page-subtitle">Tous les contenus publiés, triés du plus récent au plus ancien.</p>
    ...
    <?php if (($pagination['pages'] ?? 1) > 1): ?>
        <nav class="pagination" aria-label="Pagination des articles">
            ...
            <?php if ($current > 1): ?>
                <a href="<?= e(url_with_page($basePath ?? '/articles', $current - 1)) ?>" class="pagination-link">Précédent</a>
            <?php endif; ?>

            <?php for ($i = 1; $i <= $pages; $i++): ?>
                <a href="<?= e(url_with_page($basePath ?? '/articles', $i)) ?>" class="pagination-link <?= $i === $current ? 'is-current' : '' ?>"><?= e((string) $i) ?></a>
            <?php endfor; ?>

            <?php if ($current < $pages): ?>
                <a href="<?= e(url_with_page($basePath ?? '/articles', $current + 1)) ?>" class="pagination-link">Suivant</a>
            <?php endif; ?>
        </nav>
    <?php endif; ?>
</section>
